...
@DatasetReader.register("drop_rc_v2_fixed")
class RCDropReaderV2Fixed(DatasetReader):
    """
    ...
    def text_to_instance(self,  # type: ignore
        ...
        if not passage_tokens:
            ...
            # ADDED: keeping an unsplit version to extract numbers from
            # passage_tokens_unsplit = passage_tokens
            # passage_tokens = split_tokens_by_hyphen(passage_tokens)

        question_tokens = self._tokenizer.tokenize(question_text)
        # question_tokens_unsplit = question_tokens
        ...
        if self.instance_format == "squad":
            ...
            # ADDED: using non-hyphen-split tokens to extract numbers
            # passage_tokens_unsplit = self._tokenizer.tokenize(passage_text)

            for token_index, token in enumerate(passage_tokens):
                numbers = self.convert_word_to_number(token.text)
                # if no numbers found, returns empty list
                # potentally returning multiple numbers per index e.g. 20-10
                ...
                try:
                    ...
                    if "-" in word:
                        ...
                        for chunk in word_chunks:
                            converted_nums = RCDropReaderV2Fixed.convert_word_to_number(chunk)
